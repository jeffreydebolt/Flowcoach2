...
class GTDReviewAgent(BaseAgent):
    ...
    def _continue_weekly_review(
        self, response: str, user_id: str, current_step: int
    ) -> dict[str, Any]:
        ...
        # Update context
        self.update_context({"review_step": next_step, "review_data": review_data})
    ...
    def _review_step_2_process_inbox(self, user_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "workflow": {"step": 2, "checklist": self.review_steps.get("process_inboxes", [])},
        }
    ...
    def _generate_insights(self, user_id: str, insight_type: str = "weekly") -> dict[str, Any]:
        ...
        try:
            ...
            if tasks:
                ...
                if overdue_count > 0:
                    insights.append(
                        f"⚠️ **Attention:** {overdue_count} overdue tasks need immediate attention"
                    )

                if contexts:
                    top_context = max(contexts, key=contexts.get)
                    insights.append(
                        f"📍 **Context Focus:** Most tasks are in {top_context} ({contexts[top_context]} tasks)"
                    )
                ...
                if projects:
                    ...
                    insights.append(
                        f"🎯 **Active Projects:** {len(active_projects)} projects in progress"
                    )
            else:
            ...
            if not contexts:
                recommendations.append(
                    "• Try adding GTD contexts (@computer, @phone, @errands) to your tasks"
                )

            recommendations_text = (
                "\n".join(recommendations)
                if recommendations
                else "• Keep up the great work with your GTD system!"
            )
    ...
    def _get_productivity_analytics(self, user_id: str, period: str) -> dict[str, Any]:
        ...
        return {
            ...
            "context_distribution": {"@computer": 15, "@phone": 4, "@office": 3, "@errands": 1},
            "project_stats": {"active": 5, "completed": 2, "stalled": 1},
    ...
    def _generate_analytics_insights(self, analytics: dict[str, Any], period: str) -> str:
        ...
        if completion_rate > 0.8:
            insights.append(
                "• Excellent completion rate! You're staying on top of your commitments."
            )
    ...
    def _parse_time_threshold(self, threshold: str) -> int:
        ...
        threshold_map = {"1week": 7, "2weeks": 14, "1month": 30, "3days": 3, "1day": 1}
    ...
    def _generate_review_completion_insights(
        self, user_id: str, review_data: dict[str, Any]
    ) -> str:
