...
def test_time_extraction():
    ...
    test_cases = [
        ...
        (
            "email Aaron about cash flow questions 2 mins",
            ("2min", "email Aaron about cash flow questions"),
        ),
        (
            "build cash flow forecast for client a 30 mins plus",
            ("30+min", "build cash flow forecast for client a"),
        ),
        ...
        (
            "long planning session for Q1 strategy",
            ("30+min", "long planning session for Q1 strategy"),
        ),
    ]
    ...
    for text, expected in test_cases:
        ...
        print("✅ PASS" if (time_est, cleaned) == expected else "❌ FAIL")
...
def test_project_detection():
    ...
    services = {"openai": openai_service, "todoist": todoist_service}
...
def test_full_flow():
    ...
    message = {
        "text": "build comprehensive financial model for startup - 30 mins +",
        "user": "test_user",
    }
    response = agent.process_message(message, {})
    print(f"Response type: {response.get('response_type')}")
