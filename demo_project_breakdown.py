...
from workflows.project_breakdown import ProjectBreakdownWorkflow, register_project_breakdown_workflow
...
class ProjectBreakdownDemo:
    ...
    def _simulate_planning_process(self, initial_planning_response: Dict[str, Any]):
        ...
        if final_response.get("response_type") == "planning_complete":
            print(f"   ✅ Planning Complete! {len(final_response.get('next_actions', []))} next actions identified")
            ...
            if completion_response.get("response_type") == "project_breakdown_complete":
                print(f"✅ {completion_response.get('tasks_created', 0)} tasks created successfully!")
                ...
                review_response = self.project_workflow.handle_workflow_action(
                    user_id=self.demo_user,
                    action="schedule_project_review"
                )
