...
class TestHealthChecker(unittest.TestCase):
    ...
    def test_overall_status_determination(self):
        ...
        status = self.health_checker._determine_overall_status(
            ...
            services_status={"todoist": "configured"}
        )
        ...
        status = self.health_checker._determine_overall_status(
            db_status="ok",
            critical_error_count=0,
            services_status={"todoist": "not_configured"}
        )
        self.assertEqual(status, "degraded")
...
class TestHealthStatusSerialization(unittest.TestCase):
    ...
    def test_to_dict_conversion(self):
        ...
        expected_fields = [
            'status', 'uptime_seconds', 'last_error_time', 'error_count_24h',
            'critical_error_count_24h', 'database_status', 'services_status', 'timestamp'
        ]
