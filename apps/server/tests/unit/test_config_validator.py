...
from apps.server.core.config_validator import ConfigValidator, ValidationResult, validate_startup_config
...
class TestConfigValidator(unittest.TestCase):
    ...
    def test_time_buckets_validation(self):
        ...
        result = ConfigValidator.validate_config(env_vars)
        self.assertTrue(any('FC_TIME_BUCKETS format invalid' in warning for warning in result.warnings))
    ...
    def test_timezone_validation(self):
        ...
        for timezone in valid_timezones:
            with self.subTest(timezone=timezone):
                ...
                result = ConfigValidator.validate_config(env_vars)
                timezone_warnings = [w for w in result.warnings if 'FC_DEFAULT_TIMEZONE may be invalid' in w]
                self.assertEqual(len(timezone_warnings), 0, f"Valid timezone {timezone} should not generate warnings")
        ...
        result = ConfigValidator.validate_config(env_vars)
        self.assertTrue(any('FC_DEFAULT_TIMEZONE may be invalid' in warning for warning in result.warnings))
    ...
    def test_log_level_validation(self):
        ...
        for level in valid_levels:
            with self.subTest(level=level):
                ...
                self.assertEqual(len(log_warnings), 0, f"Valid log level {level} should not generate warnings")
