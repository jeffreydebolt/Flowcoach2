...
class TestMomentumIntegration:
    ...
    def test_task_completion_workflow(self):
        ...
        self.conn.execute("UPDATE project_momentum SET momentum_score = 50 WHERE project_id = ?", (project_id,))
        self.conn.commit()
    ...
    def test_mixed_project_decay_scenario(self):
        ...
        for project in projects:
            self.conn.execute("""
                ...
                VALUES (?, ?, ?, ?)
            """, (project["id"], project["last_activity"].isoformat(),
                  project["score"], project["status"]))
