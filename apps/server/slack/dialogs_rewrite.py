...
class ProjectRewriteDialog:
    ...
    def start_rewrite_flow(self, user_id: str, project_id: str, project_name: str, client: WebClient):
    ...
    def _handle_outcome_response(self, state: Dict, outcome_text: str, client: WebClient) -> bool:
        ...
        try:
            ...
            client.chat_postMessage(
                channel=user_id,
                text=f":calendar: Great! Now when should this be completed?",
                blocks=[
                    {
                        "type": "section",
                        "text": {
    ...
    def _complete_rewrite(self, state: Dict, outcome: str, due_date: datetime, client: WebClient) -> bool:
        ...
        try:
            ...
            try:
                ...
                self.tracker.update_project_outcome(project_id, outcome_defined=True, due_date=due_date)
                ...
                client.chat_postMessage(
                    channel=user_id,
                    text=f":tada: Successfully rewrote your project!",
                    blocks=[
                        {
                            "type": "section",
                            "text": {
    ...
    def _find_active_state(self, user_id: str) -> Optional[str]:
        """Find active rewrite state for user."""
        for state_key, state in self._rewrite_state.items():
            if state['user_id'] == user_id and state['step'] in ['awaiting_outcome', 'awaiting_due_date']:
                return state_key
        return None
    ...
    def cancel_rewrite_flow(self, user_id: str, client: WebClient) -> bool:
        ...
        try:
            ...
            client.chat_postMessage(
                channel=user_id,
                text=f":x: Cancelled rewrite flow for *{project_name}*."
            )
...
def start_rewrite_for_project(user_id: str, project_id: str, project_name: str, client: WebClient) -> bool:
...
def register_rewrite_handlers(app: App):
    ...
    def handle_cancel_command(ack, command, client):
        ...
        if not cancelled:
            client.chat_postMessage(
                channel=user_id,
                text=":shrug: No active rewrite flow to cancel."
            )
