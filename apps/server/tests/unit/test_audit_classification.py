...
class TestAuditClassification:
    ...
    def test_healthy_project_classification(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            projects = [{"id": 123, "name": "Website Redesign"}]
    ...
    def test_needs_definition_classification(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            projects = [{"id": 456, "name": "Research Project"}]
    ...
    def test_stalled_project_classification(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            projects = [{"id": 789, "name": "Mobile App"}]
    ...
    def test_new_project_without_momentum_data(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            projects = [{"id": 999, "name": "New Project"}]
    ...
    def test_mixed_project_portfolio(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            momentum_data = {
                "100": ProjectMomentum(
                    "100", datetime.now(), 85, "active", True, datetime.now() + timedelta(days=7)
                ),  # Healthy
                "200": ProjectMomentum(
                    "200", datetime.now() - timedelta(days=2), 65, "active", False, None
                ),  # Needs definition
                "300": ProjectMomentum(
                    "300",
    ...
    def test_project_sorting_by_momentum(self):
        ...
        with patch.object(self.auditor.tracker, "get_project_momentum") as mock_get_momentum:
            ...
            momentum_data = {
                "1": ProjectMomentum(
                    "1", datetime.now(), 90, "active", True, datetime.now() + timedelta(days=7)
                ),
                "2": ProjectMomentum(
                    "2", datetime.now(), 70, "active", True, datetime.now() + timedelta(days=14)
                ),
                "3": ProjectMomentum(
                    "3", datetime.now(), 85, "active", True, datetime.now() + timedelta(days=21)
                ),
            }
