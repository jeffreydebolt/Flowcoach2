...
class GTDPlanningAgent(BaseAgent):
    ...
    def __init__(self, config: Dict[str, Any], services: Dict[str, Any] = None):
        ...
        if not self.has_todoist:
            logger.warning(f"{self.name}: Todoist service not available - task creation will be limited")
        if not self.has_openai:
            logger.warning(f"{self.name}: OpenAI service not available - AI features will be limited")
        if not self.has_persistence:
            logger.warning(f"{self.name}: Workflow persistence not available - workflows won't survive restarts")
        ...
        self.planning_steps = self.planning_config.get("steps", [
    ...
    def can_handle(self, message: Dict[str, Any]) -> bool:
        ...
        planning_indicators = [
            "break down", "breakdown", "plan out", "help me organize",
            "where do i start", "steps for", "project plan", "how do i"
        ]
        ...
        clarification_indicators = [
            "what does success look like", "help me clarify", "not sure what i want",
            "vague idea", "general goal"
        ]
        ...
        complexity_indicators = [
            "complex", "overwhelming", "big project", "lots of moving parts"
        ]
    ...
    def cmd_breakdown(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def cmd_clarify_outcome(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Clarify project outcome and success criteria."""
        user_id = message.get("user", "unknown")
        return self._initiate_outcome_clarification(args, user_id)
    
    def cmd_brainstorm(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Facilitate brainstorming session."""
        user_id = message.get("user", "unknown")
        return self._facilitate_brainstorm(args, user_id)
    
    def cmd_organize_tasks(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Organize brainstormed items into structure."""
        user_id = message.get("user", "unknown")
        return self._organize_brainstormed_items(user_id)
    
    def cmd_next_actions(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Extract immediate next actions."""
        user_id = message.get("user", "unknown")
        return self._extract_next_actions(args, user_id)
    
    def cmd_estimate_project(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
            ...
            if template:
                return {
                    ...
                    "actions": [
                        ...
                        {"label": "👀 Preview Template", "value": f"preview_template_{suggested_template}"}
                    ],
                    "context_update": {
    ...
    def _handle_brainstorm_input(self, brainstorm_text: str, user_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "workflow": {
                "step": 4,
                "total_steps": 5,
                "organized_items": len(brainstorm_items)
            }
        }
    ...
    def _auto_organize_items(self, items: List[str]) -> Dict[str, Any]:
        ...
        for item in items:
            ...
            if any(word in item_lower for word in ["research", "plan", "analyze", "study", "investigate"]):
                categories["Planning & Research"].append(item)
            elif any(word in item_lower for word in ["build", "create", "develop", "design", "write", "code"]):
                categories["Development & Creation"].append(item)
            elif any(word in item_lower for word in ["email", "call", "meeting", "discuss", "coordinate", "contact"]):
                categories["Communication & Coordination"].append(item)
            elif any(word in item_lower for word in ["test", "review", "check", "validate", "verify", "quality"]):
                categories["Testing & Quality"].append(item)
            elif any(word in item_lower for word in ["launch", "deploy", "release", "publish", "go live"]):
    ...
    def cmd_resume(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        workflow_list = "\n".join([
            f"• {w['workflow_id']} (updated: {w['updated_at']})"
            for w in active_workflows[:5]
        ])
    ...
    def _suggest_template(self, project_description: str) -> Optional[str]:
    ...
    def cmd_template(self, args: str, message: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        parts = args.split(" ", 1)
        template_id = parts[0]
        project_name = parts[1] if len(parts) > 1 else f"New {template_id.replace('_', ' ').title()}"
    ...
    def _apply_template(self, template: Dict[str, Any], project_name: str, user_id: str) -> Dict[str, Any]:
